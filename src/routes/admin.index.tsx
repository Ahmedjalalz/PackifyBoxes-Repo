...
function AdminPanel() {
  ...
  const metrics = useMemo(() => {
    ...
    return { total: products.length, published, featured, drafts };
  }, [products]);
    ...
    <div className="min-h-screen bg-linear-to-b from-background via-background to-surface/40">
      <header className="sticky top-0 z-30 border-b bg-background/85 backdrop-blur-xl">
        <div className="container-tight flex items-center justify-between gap-4 py-4">
          ...
          <Button variant="outline" onClick={async () => { await signOutAdmin(); navigate({ to: "/admin/login" }); }}>
      ...
      <main className="container-tight space-y-8 py-8 md:py-10">
        <section className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-ink-muted">Manage products and categories from a single product-focused workspace.</p>
            </div>
          </div>
        ...
        <section className="space-y-5">
          <SectionHeader title="Products" description="Search, filter, edit, publish, and delete products." actions={<Button className="bg-gradient-accent border-0 shadow-soft hover:shadow-glow" onClick={() => { setEditorProductId(null); setEditorOpen(true); }}>Add product</Button>} />
          <div className="grid gap-3 rounded-3xl border bg-card p-4 shadow-soft lg:grid-cols-[1.4fr_0.8fr_0.8fr]">
            <div className="relative">
              ...
              <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products, slugs, descriptions..." className="pl-9" />
            </div>
          ...
          {productsLoading ? (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-105 rounded-3xl" />)}
            </div>
          ) : (
            <>
              ...
              <div className="hidden overflow-hidden rounded-3xl border bg-card shadow-soft md:block">
                <Table>
                  ...
                  <TableBody>
                    {pageItems.length === 0 ? (
                      <TableRow><TableCell colSpan={6} className="py-14 text-center text-muted-foreground">No products found.</TableCell></TableRow>
                    ) : pageItems.map((product) => (
                      <TableRow key={product.id}>
                        ...
                        <TableCell className="text-right">
                          <div className="inline-flex items-center gap-1">
                            <Button type="button" variant="ghost" size="sm" onClick={() => { setEditorProductId(product.id); setEditorOpen(true); }}>Edit</Button>
                            <Button type="button" variant="ghost" size="icon" onClick={() => setDeleteTarget(product.id)}><Trash2 className="h-4 w-4" /></Button>
                          </div>
                        </TableCell>
                      </TableRow>
              ...
              <div className="flex items-center justify-between gap-3 rounded-3xl border bg-card p-4 shadow-soft">
                <p className="text-sm text-muted-foreground">Showing {pageItems.length} of {filteredProducts.length} products</p>
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setPage((value) => Math.min(totalPages, value + 1))} disabled={currentPage === totalPages}>Next</Button>
                </div>
              </div>
            </>
          )}
        ...
        <section className="space-y-5 pb-10">
          <SectionHeader title="Categories" description="Create categories here and they will immediately appear in the product category dropdown." />
          <div className="grid gap-5 lg:grid-cols-[380px_minmax(0,1fr)]">
            <div className="rounded-3xl border bg-card p-5 shadow-soft">
              ...
              <div className="mt-5 space-y-4">
                ...
                <Button className="w-full bg-gradient-accent border-0 shadow-soft hover:shadow-glow" onClick={async () => {
                  const name = categoryName.trim();
                  const slug = slugify(name);
                  if (!name || !slug) { toast.error("Category name is required"); return; }
                  try {
                    await createCategory({ data: { name, slug, description: null } });
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {categoriesLoading ? Array.from({ length: 3 }).map((_, index) => <Skeleton key={index} className="h-40 rounded-3xl" />) : categories.map((category) => (
                <div key={category.id} className="rounded-3xl border bg-card p-5 shadow-soft">
                  ...
                  <div className="mt-4 flex items-center gap-3 text-xs text-muted-foreground"><Tag className="h-4 w-4" /> {category.slug}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <ProductEditorDialog open={editorOpen} onOpenChange={(open) => { setEditorOpen(open); if (!open) setEditorProductId(null); }} product={editorProduct} categories={categories} onSaved={async () => { await qc.invalidateQueries({ queryKey: ["admin", "products"] }); }} />

      <Dialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Delete product?</DialogTitle>
            <DialogDescription>This removes the product and all related content sections, styles, materials, features, and gallery images.</DialogDescription>
          </DialogHeader>
          <DialogFooter>
...
function MetricCard({ title, value, note, icon: Icon }: { title: string; value: number; note: string; icon: ComponentType<{ className?: string }>; }) {
  return (
